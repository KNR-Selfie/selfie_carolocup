...
void ChangeLaneLogic::check_lane_status(geometry_msgs::Polygon polygon){
    ...
    static uint8_t current_on_right = 0;
    if (line_coef.center[0]>0)
    ...
    for (int i=0; i<4; ++i){
        if (!current_on_right && check_point(polygon.points[i].x,polygon.points[i].y, line_coef.left[2],line_coef.left[1],line_coef.left[0], line_coef.center[2],line_coef.center[1],line_coef.center[0]))
            left_points_tmp++;
        if (current_on_right && check_point(polygon.points[i].x,polygon.points[i].y, line_coef.center[2],line_coef.center[1],line_coef.center[0], line_coef.right[2],line_coef.right[1],line_coef.right[0]))
            right_points_tmp++;
